...
export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) => {
  try {
    ...
    const { results } = await env.DB.prepare(
      `SELECT id, contract_id AS contractId, date, type, description, phase, created_at AS createdAt
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  try {
    await requireAuth(request, env);
    const id = String(params.id || "");
    const body = await readJson<{ date?: string; type?: string; description?: string; phase?: string }>(request);
    const hid = crypto.randomUUID();
    const date = (body.date || "").trim() || new Date().toISOString().slice(0, 10);
    await env.DB.prepare(
      `INSERT INTO project_history (id, contract_id, date, type, description, phase)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(hid, id, date, (body.type || "observacao").trim(), (body.description || "").trim(), (body.phase || "").trim() || null).run();
