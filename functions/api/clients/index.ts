...
const COLS = "id, name, cpf_cnpj, email, phone, address, city, state, role, nacionalidade, birth_date, photo_url, avatar, gender, created_at AS createdAt, updated_at AS updatedAt";
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO clients (id, name, cpf_cnpj, email, phone, address, city, state, role, nacionalidade, birth_date, gender)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
      .bind(id, name, cpf_cnpj, email, opt(body.phone), opt(body.address), opt(body.city), opt(body.state),
        opt(body.role), opt(body.nacionalidade), opt(body.birth_date), opt(body.gender))
      .run();
