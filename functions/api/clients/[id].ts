...
export const onRequestPut: PagesFunction<Env> = async ({ request, env, params }) => {
  try {
    ...
    await env.DB.prepare(
      `UPDATE clients
       SET name = ?, cpf_cnpj = ?, email = ?, phone = ?, address = ?, city = ?, state = ?, updated_at = datetime('now')
       WHERE id = ?`
    )
      .bind(name, cpf_cnpj, email, opt(body.phone), opt(body.address), opt(body.city), opt(body.state), id)
      .run();
